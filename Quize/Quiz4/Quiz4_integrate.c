...
void menu(){
    ...
    printf("\n12 : Delete record");
    printf("\n13 : List all females under 45 or lives in Pekin");
    printf("\n14 : Exit.");
...
void viewsp(){
    FILE *fp;
    int i=1,j;
    char stgender[SIZE] = "f"; //female
    char stcity[SIZE] = "Pekin"; //city
    struct student srh;
    printf("\n\n<--:LIST ALL FEMALES UNDER 45 OR LIVES IN PEKIN:-->");
    printf("\nname\tLastname\tAge\tGender\tSt.name\t\tSt.No.\tCity\tCountry\t\tPostcode: ");
    ...
    j=8;
    while(fread(&srh,sizeof(srh),1,fp) == 1){
        if((strcmp(stgender,srh.gender) == 0 && srh.age<45) || (strcmp(stcity,srh.x.city) == 0 && strcmp(stgender,srh.gender) == 0)){ ///younger than 25 or live in Pekin and female
        printf("\n%s\t%s\t%d\t%s\t%s\t%d\t%s\t%s\t\t%d",srh.fname,srh.lname,srh.age,srh.gender,srh.x.streetName,srh.x.streetNumber,srh.x.city,srh.x.country,srh.x.zipCode);
        i++;
        j++;
        }
